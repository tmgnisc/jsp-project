<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Nepali Movies - Nepal Navigator</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        body {
            font-family: 'Poppins', sans-serif;
        }
    </style>
</head>
<body class="bg-gray-50">
     <!-- Navigation -->
    <nav class="bg-[#002B5B] text-white shadow-lg">
        <div class="container mx-auto px-4">
            <div class="flex justify-between items-center py-4">
                <a href="index" class="text-2xl font-bold text-[#F4A300]">Nepal Navigator</a>
                <div class="hidden md:flex space-x-6">
                    <a href="index" class="hover:text-[#F4A300]">Home</a>
                    <a href="foods" class="hover:text-[#F4A300]">Foods</a>
                    <a href="scenery" class="hover:text-[#F4A300]">Attractions</a>
                    <a href="music" class="hover:text-[#F4A300]">Music</a>
                    <a href="movies" class="hover:text-[#F4A300]">Movies</a>
                    <a href="sport" class="hover:text-[#F4A300]">Sports</a>
                    <!-- Dynamically change based on login state -->
                    <% 
                        String username = (String) session.getAttribute("username");
                        if (username != null) { 
                    %>
                        <span class="text-white">Welcome, <%= username %>!</span>
                        <a href="logout" class="hover:text-[#F4A300]">Logout</a>
                    <% } else { %>
                        <a href="login" class="hover:text-[#F4A300]">Login/Register</a>
                    <% } %>
                </div>
                <button class="md:hidden">
                    <i class="fas fa-bars text-2xl"></i>
                </button>
            </div>
        </div>
    </nav>

    <!-- Hero Section -->
    <div class="relative h-[300px] bg-cover bg-center" style="background-image: url('https://images.unsplash.com/photo-1544735716-392fe2489ffa?ixlib=rb-1.2.1&auto=format&fit=crop&w=1950&q=80');">
        <div class="absolute inset-0 bg-black bg-opacity-50"></div>
        <div class="relative container mx-auto px-4 h-full flex items-center">
            <div class="text-white">
                <h1 class="text-4xl font-bold mb-4">Nepali Movies</h1>
                <p class="text-xl">Explore the rich cinematic heritage of Nepal</p>
            </div>
        </div>
    </div>

    <!-- Search Section -->
    <div class="bg-white shadow-md py-6">
        <div class="container mx-auto px-4">
            <div class="max-w-3xl mx-auto">
                <div class="flex flex-col md:flex-row gap-4">
                    <div class="flex-1">
                        <input type="text" placeholder="Search for movies..." class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#F4A300] focus:border-transparent">
                    </div>
                    <div class="flex gap-4">
                        <select class="px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#F4A300] focus:border-transparent">
                            <option value="">All Genres</option>
                            <option value="drama">Drama</option>
                            <option value="comedy">Comedy</option>
                            <option value="action">Action</option>
                            <option value="romance">Romance</option>
                        </select>
                        <button class="bg-[#F4A300] text-white px-6 py-2 rounded-md hover:bg-[#A31621] transition duration-300">
                            <i class="fas fa-search mr-2"></i>Search
                        </button>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Movies Grid -->
    <div class="container mx-auto px-4 py-12">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <!-- Movie Card 1 -->
            <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                <img src="https://images.unsplash.com/photo-1544735716-392fe2489ffa?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=80" alt="Movie 1" class="w-full h-48 object-cover">
                <div class="p-6">
                    <div class="flex justify-between items-start">
                        <div>
                            <h3 class="text-xl font-semibold text-[#002B5B] mb-2">Nepali Classic</h3>
                            <p class="text-gray-600 mb-2">A masterpiece of Nepali cinema</p>
                            <p class="text-sm text-gray-500"><i class="fas fa-star mr-2"></i>4.5/5</p>
                        </div>
                        <span class="bg-[#F4A300] text-white px-3 py-1 rounded-full text-sm">Drama</span>
                    </div>
                    
                    <!-- Movie Links -->
                    <div class="mt-4 space-y-2">
                        <a href="#" class="flex items-center text-[#F4A300] hover:text-[#A31621]">
                            <i class="fab fa-youtube mr-2"></i>
                            <span>Watch Trailer</span>
                        </a>
                        <a href="#" class="flex items-center text-[#F4A300] hover:text-[#A31621]">
                            <i class="fas fa-ticket-alt mr-2"></i>
                            <span>Book Tickets</span>
                        </a>
                    </div>
                    
                    <!-- Comments Section -->
                    <div class="mt-6">
                        <h4 class="font-semibold text-gray-700 mb-3">Comments</h4>
                        <div class="space-y-4">
                            <div class="bg-gray-50 p-3 rounded">
                                <div class="flex items-center mb-2">
                                    <img src="https://ui-avatars.com/api/?name=John+Doe" alt="User" class="w-8 h-8 rounded-full mr-2">
                                    <div>
                                        <p class="font-medium text-sm">John Doe</p>
                                        <p class="text-xs text-gray-500">2 days ago</p>
                                    </div>
                                </div>
                                <p class="text-sm text-gray-600">A must-watch for anyone interested in Nepali cinema!</p>
                            </div>
                        </div>
                        
                        <!-- Comment Input -->
                        <div class="mt-4">
                            <textarea placeholder="Write a comment..." class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#F4A300] focus:border-transparent"></textarea>
                            <button class="mt-2 bg-[#F4A300] text-white px-4 py-2 rounded-md hover:bg-[#A31621] transition duration-300">
                                Post Comment
                            </button>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Movie Card 2 -->
            <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                <img src="https://images.unsplash.com/photo-1544735716-392fe2489ffa?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=80" alt="Movie 2" class="w-full h-48 object-cover">
                <div class="p-6">
                    <div class="flex justify-between items-start">
                        <div>
                            <h3 class="text-xl font-semibold text-[#002B5B] mb-2">Modern Nepali Film</h3>
                            <p class="text-gray-600 mb-2">Contemporary Nepali cinema</p>
                            <p class="text-sm text-gray-500"><i class="fas fa-star mr-2"></i>4.2/5</p>
                        </div>
                        <span class="bg-[#F4A300] text-white px-3 py-1 rounded-full text-sm">Comedy</span>
                    </div>
                    
                    <!-- Movie Links -->
                    <div class="mt-4 space-y-2">
                        <a href="#" class="flex items-center text-[#F4A300] hover:text-[#A31621]">
                            <i class="fab fa-youtube mr-2"></i>
                            <span>Watch Trailer</span>
                        </a>
                        <a href="#" class="flex items-center text-[#F4A300] hover:text-[#A31621]">
                            <i class="fas fa-ticket-alt mr-2"></i>
                            <span>Book Tickets</span>
                        </a>
                    </div>
                    
                    <!-- Comments Section -->
                    <div class="mt-6">
                        <h4 class="font-semibold text-gray-700 mb-3">Comments</h4>
                        <div class="space-y-4">
                            <div class="bg-gray-50 p-3 rounded">
                                <div class="flex items-center mb-2">
                                    <img src="https://ui-avatars.com/api/?name=Jane+Smith" alt="User" class="w-8 h-8 rounded-full mr-2">
                                    <div>
                                        <p class="font-medium text-sm">Jane Smith</p>
                                        <p class="text-xs text-gray-500">1 week ago</p>
                                    </div>
                                </div>
                                <p class="text-sm text-gray-600">Great modern take on Nepali storytelling!</p>
                            </div>
                        </div>
                        
                        <!-- Comment Input -->
                        <div class="mt-4">
                            <textarea placeholder="Write a comment..." class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#F4A300] focus:border-transparent"></textarea>
                            <button class="mt-2 bg-[#F4A300] text-white px-4 py-2 rounded-md hover:bg-[#A31621] transition duration-300">
                                Post Comment
                            </button>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Movie Card 3 -->
            <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                <img src="https://images.unsplash.com/photo-1544735716-392fe2489ffa?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=80" alt="Movie 3" class="w-full h-48 object-cover">
                <div class="p-6">
                    <div class="flex justify-between items-start">
                        <div>
                            <h3 class="text-xl font-semibold text-[#002B5B] mb-2">Documentary</h3>
                        <p class="text-gray-600 mb-2">An insightful documentary about Nepal</p>
                        <p class="text-sm text-gray-500"><i class="fas fa-star mr-2"></i>4.8/5</p>
                        </div>
                        <span class="bg-[#F4A300] text-white px-3 py-1 rounded-full text-sm">Documentary</span>
                    </div>
                    
                    <!-- Movie Links -->
                    <div class="mt-4 space-y-2">
                        <a href="#" class="flex items-center text-[#F4A300] hover:text-[#A31621]">
                            <i class="fab fa-youtube mr-2"></i>
                            <span>Watch Trailer</span>
                        </a>
                        <a href="#" class="flex items-center text-[#F4A300] hover:text-[#A31621]">
                            <i class="fas fa-ticket-alt mr-2"></i>
                            <span>Book Tickets</span>
                        </a>
                    </div>
                    
                    <!-- Comments Section -->
                    <div class="mt-6">
                        <h4 class="font-semibold text-gray-700 mb-3">Comments</h4>
                        <div class="space-y-4">
                            <div class="bg-gray-50 p-3 rounded">
                                <div class="flex items-center mb-2">
                                    <img src="https://ui-avatars.com/api/?name=Mike+Johnson" alt="User" class="w-8 h-8 rounded-full mr-2">
                                    <div>
                                        <p class="font-medium text-sm">Mike Johnson</p>
                                        <p class="text-xs text-gray-500">3 days ago</p>
                                    </div>
                                </div>
                                <p class="text-sm text-gray-600">Fascinating insights into Nepali culture and history!</p>
                            </div>
                        </div>
                        
                        <!-- Comment Input -->
                        <div class="mt-4">
                            <textarea placeholder="Write a comment..." class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#F4A300] focus:border-transparent"></textarea>
                            <button class="mt-2 bg-[#F4A300] text-white px-4 py-2 rounded-md hover:bg-[#A31621] transition duration-300">
                                Post Comment
                            </button>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <!-- Upcoming Movies Section -->
        <div class="mt-16">
            <h2 class="text-3xl font-bold text-[#002B5B] mb-8">Coming Soon</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <!-- Upcoming Movie 1 -->
                <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                    <div class="p-6">
                        <h3 class="text-xl font-semibold text-[#002B5B] mb-2">New Nepali Film</h3>
                        <p class="text-gray-600 mb-2">Release Date: June 1, 2024</p>
                        <p class="text-gray-600 mb-4">Genre: Action/Drama</p>
                        <a href="#" class="text-[#F4A300] hover:text-[#A31621]">Learn More →</a>
                    </div>
                </div>

                <!-- Upcoming Movie 2 -->
                <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                    <div class="p-6">
                        <h3 class="text-xl font-semibold text-[#002B5B] mb-2">Comedy Special</h3>
                        <p class="text-gray-600 mb-2">Release Date: July 15, 2024</p>
                        <p class="text-gray-600 mb-4">Genre: Comedy</p>
                        <a href="#" class="text-[#F4A300] hover:text-[#A31621]">Learn More →</a>
                    </div>
                </div>

                <!-- Upcoming Movie 3 -->
                <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                    <div class="p-6">
                        <h3 class="text-xl font-semibold text-[#002B5B] mb-2">Historical Drama</h3>
                        <p class="text-gray-600 mb-2">Release Date: August 30, 2024</p>
                        <p class="text-gray-600 mb-4">Genre: Historical/Drama</p>
                        <a href="#" class="text-[#F4A300] hover:text-[#A31621]">Learn More →</a>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Footer -->
    <footer class="bg-[#002B5B] text-white py-12">
        <div class="container mx-auto px-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                <div>
                    <h3 class="text-xl font-bold text-[#F4A300] mb-4">Nepal Navigator</h3>
                    <p class="text-gray-300">Discover the beauty and culture of Nepal through our comprehensive guide.</p>
                </div>
                <div>
                    <h4 class="text-lg font-semibold mb-4">Quick Links</h4>
                    <ul class="space-y-2">
                        <li><a href="index.html" class="text-gray-300 hover:text-[#F4A300]">Home</a></li>
                        <li><a href="foods.html" class="text-gray-300 hover:text-[#F4A300]">Foods</a></li>
                        <li><a href="scenery.html" class="text-gray-300 hover:text-[#F4A300]">Attractions</a></li>
                        <li><a href="music.html" class="text-gray-300 hover:text-[#F4A300]">Music</a></li>
                    </ul>
                </div>
                <div>
                    <h4 class="text-lg font-semibold mb-4">Contact Us</h4>
                    <ul class="space-y-2 text-gray-300">
                        <li><i class="fas fa-envelope mr-2"></i> info@nepalnavigator.com</li>
                        <li><i class="fas fa-phone mr-2"></i> +977 1234567890</li>
                        <li><i class="fas fa-map-marker-alt mr-2"></i> Kathmandu, Nepal</li>
                    </ul>
                </div>
                <div>
                    <h4 class="text-lg font-semibold mb-4">Follow Us</h4>
                    <div class="flex space-x-4">
                        <a href="#" class="text-gray-300 hover:text-[#F4A300]"><i class="fab fa-facebook-f"></i></a>
                        <a href="#" class="text-gray-300 hover:text-[#F4A300]"><i class="fab fa-twitter"></i></a>
                        <a href="#" class="text-gray-300 hover:text-[#F4A300]"><i class="fab fa-instagram"></i></a>
                        <a href="#" class="text-gray-300 hover:text-[#F4A300]"><i class="fab fa-youtube"></i></a>
                    </div>
                </div>
            </div>
            <div class="border-t border-gray-700 mt-8 pt-8 text-center text-gray-300">
                <p>&copy; 2024 Nepal Navigator. All rights reserved.</p>
            </div>
        </div>
    </footer>
</body>
</html> 