...
<head>
    ...
    <title>Celebrities Management - Nepal Navigator</title>
...
%>
        ...
        <div class="flex-1 overflow-auto">
            <div class="bg-white shadow-md">
                <div class="flex justify-between items-center px-8 py-4">
                    <h1 class="text-2xl font-semibold text-[#002B5B]">Celebrities Management</h1>
            ...
            <div class="p-8">
                <div class="mb-6">
                    <button onclick="showAddCelebrityModal()" class="bg-[#F4A300] text-white px-4 py-2 rounded-md hover:bg-[#A31621] transition duration-300">
                        <i class="fas fa-plus mr-2"></i>Add New Celebrity
                    </button>
                </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <img src="${pageContext.request.contextPath}<%=item.getImage() != null ? item.getImage() : "/images/placeholder.jpg"%>" alt="Celebrity" class="w-16 h-16 object-cover rounded" onerror="this.src='https://via.placeholder.com/100'">
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    ...
                                    <form action="${pageContext.request.contextPath}/celebrity-dashboard" method="post" style="display:inline;">
                                        <input type="hidden" name="action" value="delete">
                                        <input type="hidden" name="id" value="<%=item.getId()%>">
                                        <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Are you sure you want to delete this celebrity?')">
    ...
    <div id="celebrityModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full">
        <div class="relative top-20 mx-auto p-5 border w-[600px] shadow-lg rounded-md bg-white">
            <div class="mt-3">
                <h3 class="text-lg font-medium text-[#002B5B] mb-4" id="modalTitle">Add New Celebrity</h3>
                <form id="celebrityForm" action="${pageContext.request.contextPath}/celebrity-dashboard" method="post" enctype="multipart/form-data" class="space-y-4">
                    <input type="hidden" name="action" id="formAction" value="add">
                    <input type="hidden" name="id" id="celebrityId" value="0">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Celebrity Image</label>
                    ...
                    <div>
                        ...
                        <textarea name="bio" id="bio" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#F4A300] focus:border-[#F4A300] sm:text-sm" rows="3" required></textarea>
                    </div>
                    <div class="flex justify-end space-x-3">
                        <button type="button" onclick="closeCelebrityModal()" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">
                            Cancel
                        </button>
    ...
    <script>
        function showAddCelebrityModal() {
            document.getElementById('modalTitle').textContent = 'Add New Celebrity';
            document.getElementById('formAction').value = 'add';
            document.getElementById('celebrityId').value = '0';
            document.getElementById('celebrityForm').reset();
            document.getElementById('celebrityModal').classList.remove('hidden');
        }

        function showEditCelebrityModal(id) {
            document.getElementById('modalTitle').textContent = 'Edit Celebrity';
            document.getElementById('formAction').value = 'edit';
            document.getElementById('celebrityId').value = id;

            fetch('${pageContext.request.contextPath}/celebrity-dashboard?action=getCelebrity&id=' + id)
                .then(response => {
                    if (!response.ok) {
                ...
                .catch(error => {
                    ...
                    alert('Failed to load celebrity data.');
                });
        }
